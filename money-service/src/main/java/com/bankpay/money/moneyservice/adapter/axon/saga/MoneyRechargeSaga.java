package com.bankpay.money.moneyservice.adapter.axon.saga;
...
public class MoneyRechargeSaga {
    ...
    @StartSaga
    @SagaEventHandler(associationProperty = "rechargingRequestId") //메모리안에서 여러개의 saga를 구분하기 위한 구분자
    public void handle(RechargingRequestCreatedEvent event){
        ...
        SagaLifecycle.associateWith("checkRegisteredBankAccountId", checkRegisteredBankAccountId); //assocationKey 정의


        System.out.println("RechargingRequestCreatedEvent received, aggregateIdentifier: "
        ...
        // -> axon server를 통해서 -> Banking Service로 흘러들어감 ->
        // CheckRegisteredBankAccountCommand은 money Service에서만 사용하는게 아님 ->Common에서 작성
    ...
    public void handle(RequestFirmbankingFinishedEvent event, IncreaseMoneyPort increaseMoneyPort) {
        ...
        System.out.println("resultEntity 잔액 수정: "+resultEntity);
